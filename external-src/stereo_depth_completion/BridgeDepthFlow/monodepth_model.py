"""
MonodepthNet model architecture from BridgeDepthFlow.

Original source: BridgeDepthFlow/models/MonodepthModel.py

This file is a direct copy of the original model architecture with minimal
modifications (removed unused imports). The network is a ResNet-like
encoder-decoder that takes 6-channel input (left + right images concatenated)
and produces multi-scale disparity estimates.

Architecture:
- Encoder: ResNet-style with Bottleneck blocks (layers: 3, 4, 6, 3)
- Decoder: Upsampling blocks with skip connections
- Output: 4 scales of 2-channel disparity (horizontal + vertical)
  - disp_est_scale: pixel-scale disparities
  - disp_est: normalized disparities (via tanh * 0.3)
"""
...
class DispBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.cat(
            (
                disp[:, 0, :, :].unsqueeze(1) * x.shape[3],
                disp[:, 1, :, :].unsqueeze(1) * x.shape[2],
            ),
            1,
        )
...
class MonodepthNet(nn.Module):
    """
    Monocular depth estimation network from BridgeDepthFlow.

    Takes 6-channel input (left + right images concatenated along channel dim)
    and outputs multi-scale disparity maps at 4 scales.

    Returns:
        disp_est_scale: list of 4 tensors, pixel-scale disparities (N, 2, H_i, W_i)
        disp_est: list of 4 tensors, normalized disparities (N, 2, H_i, W_i)
    """
    ...
    def forward(self, x):
        ...
        disp4_scale, disp4, udisp4 = self.get_disp4(upconv4)

        upconv3 = self.up3(upconv4, skip2, udisp4)
        disp3_scale, disp3, udisp3 = self.get_disp3(upconv3)

        upconv2 = self.up2(upconv3, skip1, udisp3)
        disp2_scale, disp2, udisp2 = self.get_disp2(upconv2)

        upconv1 = self.up1(upconv2, udisp2)
        disp1_scale, disp1, udisp1 = self.get_disp1(upconv1)

        return (
            [disp1_scale, disp2_scale, disp3_scale, disp4_scale],
