...
from .networks.correlation_package import Correlation
...
class PWCDCNet(nn.Module):
    ...
    def forward(self,x):
        ...
        flow0_enlarge = F.interpolate(flow2 * 4.0, size=(H, W), mode='bilinear', align_corners=True)
        flow1_enlarge = F.interpolate(flow3 * 4.0, size=(H // 2, W // 2), mode='bilinear', align_corners=True)
        flow2_enlarge = F.interpolate(flow4 * 4.0, size=(H // 4, W // 4), mode='bilinear', align_corners=True)
        flow3_enlarge = F.interpolate(flow5 * 4.0, size=(H // 8, W // 8), mode='bilinear', align_corners=True)
