...
def get_disparity_smoothness(disp, pyramid):
    ...
    weights_x = [torch.exp(-torch.mean(torch.abs(g), dim=1, keepdim=True)) for g in image_gradients_x]
    weights_y = [torch.exp(-torch.mean(torch.abs(g), dim=1, keepdim=True)) for g in image_gradients_y]
...
def cal_grad2_error(flo, image, beta):
    ...
    weights_x = torch.exp(-10.0 * torch.mean(torch.abs(img_grad_x), dim=1, keepdim=True))
    weights_y = torch.exp(-10.0 * torch.mean(torch.abs(img_grad_y), dim=1, keepdim=True))
...
def create_outgoing_mask(flow):
    ...
    pos_x = grid_x.type(torch.FloatTensor) + flow_u.detach().cpu()
    pos_y = grid_y.type(torch.FloatTensor) + flow_v.detach().cpu()
...
def make_pyramid(image, num_scales):
    ...
    for i in range(num_scales - 1):
        ...
        scale_image.append(scaled.cuda())
