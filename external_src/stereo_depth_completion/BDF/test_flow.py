...
for batch_idx, (left, right, gt, mask, h, w) in enumerate(TestFlowLoader, 0):
    ...
    disp_ori_scale = F.interpolate(disp_est_scale[0][:1], size=(int(h), int(w)), mode='bilinear', align_corners=True)
    ...
    error, fl = evaluate_flow(disp_ori_scale[0].detach().cpu().numpy(), gt[0].numpy(), mask.numpy())
    total_error += error
    fl_error += fl
    num_test += 1
