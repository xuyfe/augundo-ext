"""Disparity evaluation -- PyTorch reimplementation (numpy-based).
Replaces scipy.misc with cv2 for image I/O.
"""
import numpy as np
import cv2
...
def eval_disp_avg(pred_disps, path, disp_num=None, moving_masks=None):
    ...
    for i, pred_disp in enumerate(pred_disps):
        if disp_num is not None:
            gt_disp = cv2.imread(
                path + "/disp_occ_" + str(disp_num) + "/" +
                str(i).zfill(6) + "_10.png", cv2.IMREAD_UNCHANGED)
            gt_disp_noc = cv2.imread(
                path + "/disp_noc_" + str(disp_num) + "/" +
                str(i).zfill(6) + "_10.png", cv2.IMREAD_UNCHANGED)
        else:
            gt_disp = cv2.imread(
                path + "/disp_occ/" + str(i).zfill(6) + "_10.png",
                cv2.IMREAD_UNCHANGED)
            gt_disp_noc = cv2.imread(
                path + "/disp_noc/" + str(i).zfill(6) + "_10.png",
                cv2.IMREAD_UNCHANGED)
        ...
        if moving_masks:
            ...
            error_static += calculate_error_rate(epe_map, gt_disp,
                                                 valid_mask * (1.0 - move_mask))
