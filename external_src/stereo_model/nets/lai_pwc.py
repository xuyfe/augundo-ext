...
# to adapt to stereo, constrain to extract only horizontal components of flow.
...
class Correlation(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        # Pad x2
        x2_pad = F.pad(x2, (md, md, md, md), mode='constant', value=0)
        
        cost = []
        
        for i in range(0, 2 * md + 1):
            for j in range(0, 2 * md + 1):
                x2_slice = x2_pad[:, :, i : i + H, j : j + W]
                # Compute correlation (mean product)
                c = torch.mean(x1 * x2_slice, dim=1, keepdim=True)
                cost.append(c)
...
class PWCDCNet(nn.Module):
    ...
    Ref: BridgeDepthFlow (Lai et al., CVPR 2019)
    """
    def __init__(self, md=4):
        ...
        # Use pure PyTorch correlation implementation
        self.corr    = Correlation(max_displacement=md)
