...
CHECKPOINT_DIR="$SENIOR_THESIS/augundo-ext/checkpoints/bridgedepthflow_stereo"
OUTPUT_DIR="$SENIOR_THESIS/augundo-ext/results/bridgedepthflow_stereo"
...
if [[ -d "$CHECKPOINT_DIR" ]]; then
  RESTORE_PTH=$(ls -t "$CHECKPOINT_DIR"/checkpoints-*/bridgedepthflow-*.pth 2>/dev/null | head -1)
  if [[ -z "$RESTORE_PTH" ]]; then
    echo "Error: no bridgedepthflow-*.pth found under $CHECKPOINT_DIR/checkpoints-*/"
    exit 1
  fi
