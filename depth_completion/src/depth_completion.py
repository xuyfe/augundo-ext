    ...
    def log_summary(self,
        ...
        with torch.no_grad():
            ...
            if output_depth0 is not None:
                ...
                # Log distribution of output depth (convert to CPU float numpy for TensorBoard histogram)
                summary_writer.add_histogram(
                    tag + '_output_depth0_distro',
                    output_depth0.detach().cpu().float().numpy(),
                    global_step=step)
            ...
            if output_depth0 is not None and sparse_depth0 is not None and validity_map0 is not None:
                ...
                # Log distribution of sparse depth (CPU float numpy for TensorBoard)
                summary_writer.add_histogram(
                    tag + '_sparse_depth0_distro',
                    sparse_depth0.detach().cpu().float().numpy(),
                    global_step=step)
            ...
            if output_depth0 is not None and ground_truth0 is not None:
                ...
                # Log distribution of ground truth (CPU float numpy for TensorBoard)
                summary_writer.add_histogram(
                    tag + '_ground_truth0_distro',
                    ground_truth0.detach().cpu().float().numpy(),
                    global_step=step)

            if pose0to1 is not None:
                # Log distribution of pose 1 to 0 translation vector (CPU float numpy for TensorBoard)
                for i, axis in enumerate(['tx', 'ty', 'tz']):
                    summary_writer.add_histogram(
                        tag + '_%s0to1_distro' % axis,
                        pose0to1[:, i, 3].detach().cpu().float().numpy(),
                        global_step=step)

            if pose0to2 is not None:
                # Log distribution of pose 2 to 0 translation vector (CPU float numpy for TensorBoard)
                for i, axis in enumerate(['tx', 'ty', 'tz']):
                    summary_writer.add_histogram(
                        tag + '_%s0to2_distro' % axis,
                        pose0to2[:, i, 3].detach().cpu().float().numpy(),
                        global_step=step)
